...
description: Как Dynamic API Platform и PyOrchestrator работают в составе WASH PRO CRM — порты, роли, общие данные и когда включать сервисы.
---
...
**Информационный бот (v1.9):** контент из **Dashboard → Автоматизация → Публикации**; рассылка подписчикам в **личных чатах**; изображения одним сообщением (фото + подпись); корректная занятость постов.
...
- переключатель **Dynamic API** / **PyOrchestrator**;
- HTTP URL и готовый конфиг для **Cursor** (без сборки);
- таблица зарегистрированных инструментов;
- прокси nginx: `/api/mcp`, `/api/pyorch-mcp/mcp`.

Отдельный stdio-сервер для агентов: `services/crm-mcp` (см. [docs/mcp.md](mcp.md)).
