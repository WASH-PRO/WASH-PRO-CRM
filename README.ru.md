...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.1.55-0d9488?style=flat-square" alt="Version">
...
- **Встроенная справка** — полноэкранная справка; пункт **Справка** внизу sidebar (v1.1.22, v1.1.29); разделы **Мастер / Приветствие / Профиль** *(v1.1.44)*
- **White-label брендинг** — название, слоган, URL логотипа в Настройках *(v1.1.44)*
- **Toast и подтверждения** — без `alert`/`confirm` браузера на критичных действиях *(v1.1.44)*
- **Полный пакет бэкапа** — MongoDB + настройки CRM + `data/` модулей *(v1.1.44)*
- **Диагностика для поддержки** — JSON-отчёт на странице «Информация» *(v1.1.44)*
...
- **Автоматизация** — новости/акции для Telegram, боты (управление / сервис / информационный **v2.2**), **MCP сервер** (Dynamic API + PyOrchestrator), **Модули** (расширения GitHub), бэкапы
- **Уведомления об обновлениях и модулях** — задачи обновления CRM и события модулей в web + Telegram (v1.1.34)
- **Починка модулей из браузера** — исправление Safari; пересборка modules-bridge в Настройках без SSH (v1.1.33)
- **Разделы на странице «Модули»** — блоки «Установленные» и «Доступные» с пагинацией (v1.1.38)
...
Dashboard (React) ──────────── nginx /api proxy ──────┘
                              ...
                              modules-bridge → каталог модулей (v1.1.30)
```
...
| `modules-bridge` | Каталог модулей, install/lifecycle, прокси UI | `127.0.0.1:3024` |
...
| [Модули](https://wash-pro.github.io/WASH-PRO-CRM/ru/modules/) | Каталог расширений GitHub, PyOrchestrator |
