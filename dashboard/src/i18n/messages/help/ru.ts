...
export const helpRu = {
  ...
  sections: {
    ...
    washes: {
      title: 'Автомойки',
      summary: 'Справочник объектов (филиалов): название, адрес, UUID для внешних площадок, привязка постов.',
      howItWorks:
        'CRUD через Dynamic API → MongoDB. Поле mapsExternalId (UUID) создаётся автоматически и используется модулем «Автомойки рядом» как Owner API external_id. Посты ссылаются на мойку.',
      example:
        'Добавляете «Мойка на Ленина» — CRM назначает UUID; модуль публикует мойку на мойка-про.рф с этим UUID.',
      regions: {
        toolbar: 'Кнопки: создать, обновить, поиск',
        table: 'Таблица автомоек (UUID под названием)',
        pager: 'Пагинация',
      },
    },
    posts: {
    ...
    setup: {
      ...
      summary: 'Первый запуск и повторный мастер: учётная запись, MQTT, интеграции, демо-данные.',
      howItWorks:
        'Открывается автоматически при первом входе администратора. Прогресс сохраняется в браузере; завершение отмечает setup выполненным. Повторный запуск — из подвала сайдбара.',
      example: 'После переустановки админ снова проходит мастер и проверяет MQTT и PyOrch.',
      regions: {
        language: 'Шаги мастера',
        sections: 'Аккаунт, MQTT, интеграции',
        updates: 'Завершение и вход в CRM',
      },
    },
    welcome: {
      title: 'Экран приветствия',
      summary: 'Однократное приветствие после первого входа с подсказками по роли.',
      howItWorks:
        'Показывается один раз на пользователя (localStorage). Объясняет навигацию и советует Обзор, Посты и Настройки.',
      example: 'Новый оператор читает уровень доступа и нажимает Войти в систему.',
      regions: {
        kpi: 'Приветствие и роль',
        revenue: 'Список подсказок',
        workload: 'Кнопка входа',
        charts: 'Брендинг',
        feed: '—',
      },
    },
    profile: {
      title: 'Профиль',
      summary: 'Ваш аккаунт: имя, email, пароль, язык интерфейса.',
      howItWorks:
        'Обновляет запись пользователя в Dynamic API. Смена пароля требует текущий пароль. Язык синхронизируется с локалью дашборда.',
      example: 'Оператор меняет отображаемое имя и переключает интерфейс на English.',
      regions: {
        users: 'Форма профиля',
        groups: 'Выбор языка',
      },
