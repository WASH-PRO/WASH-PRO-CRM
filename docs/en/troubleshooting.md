...
**Cause:** the browser (often Safari) keeps a stale `index.html` pointing at removed JS bundles (`/assets/index-….js`). Before v1.1.28 nginx could return HTML instead of 404 — the browser tried to run HTML as JS.

**Fix:**

1. **Hard reload:** `⌘⇧R` (Mac) or close the tab and open CRM again.
2. Upgrade to **v1.1.28+** and rebuild dashboard: `docker compose up -d --build dashboard`.
...
git fetch origin && git checkout v1.1.28
