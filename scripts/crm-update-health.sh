...
if docker compose $COMPOSE_FILES config --services 2>/dev/null | grep -qx 'modules-bridge'; then
  ...
  echo "[crm-update-health] modules-bridge not in compose config — skipped" >&2
  modules_ok=1
fi
...
echo "[crm-update-health] modules-bridge still down — container status:" >&2
docker compose $COMPOSE_FILES ps modules-bridge 2>&1 || true
echo "[crm-update-health] modules-bridge logs (tail):" >&2
