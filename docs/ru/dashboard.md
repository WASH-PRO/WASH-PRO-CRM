...
| Группа | Разделы |
|--------|---------|
| **Главное** | Обзор, **Состояние** (все посты, график, онлайн/оффлайн), **Система** (ресурсы сервера, CRM и версии стека) |
...
### Система (`/system`) — v1.1.16

Страница состояния платформы в **«Главное»** (не путать с группой **«Система»** — уведомления, пользователи, настройки).
...
### Целостность и исправление (`/settings`) — v1.1.17
...
| **Применить исправления** | Выбранные действия: синхронизация корня в `.env`, нормализация `DATA_DIR`, `git safe.directory`, сброс зависшего job, Mosquitto (`fix-mqtt.sh`), `init-seed` |
