...
Порт **1883** открыт на всех интерфейсах хоста — контроллеры в локальной сети подключаются к `<IP-сервера>:1883`. Обязательно задайте сложный `MQTT_PASSWORD`.

Чтобы принимать MQTT только с localhost: `MQTT_BIND=127.0.0.1` в `.env`.
