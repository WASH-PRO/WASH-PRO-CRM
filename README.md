...
- **Обзор** — KPI, графики Recharts, live-уведомления; агрегация по последним записям каждого поста
- **SCADA** — текущее состояние постов, live-таймер режима, **интерактивный график** на всю ширину
- **Объекты и посты** — автомойки, посты с серийным номером, **настройки устройства** (цены, команды MQTT)
- **Карты** — скидочные / сервисные / VIP; журнал применений NFC; типы скидок 1–5
- **Аналитика** — использование и финансы до/после инкассации
- **Система** — уведомления (web + Telegram), пользователи, группы RBAC, бэкапы, Telegram-боты, справочники, логи, профиль
...
| `mongodb`, `mosquitto`, … | Внутренние сервисы | — |
