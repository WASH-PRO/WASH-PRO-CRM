...
| **Целостность и исправление** | Мастер в Настройках: пути, `.env`, сбойные обновления; внешний `DATA_DIR` не помечается *(v1.1.19)* |
| **Обновления ПО** | Проверка релизов без GitHub-токена (git fallback), стабильное уведомление *(v1.1.18)* |
| **Система** | Ресурсы сервера, версия CRM, версии встроенных компонентов *(Главное)* (v1.1.16) |
...
| **Автоматизация** | **Информация**, **Telegram-боты** (управление / сервис / **информационный v2.2**), **MCP сервер**, бэкапы |
