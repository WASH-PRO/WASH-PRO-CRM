...
git fetch origin && git checkout v1.1.62   # нужный тег
...
## Зависший спиннер обновления (v1.1.50+)

**Симптомы:** в шапке или **Настройки → Обновления ПО** крутится спиннер бесконечно; задача в статусе `queued`, хотя целевая версия уже установлена.

**Причина:** зависшая задача в `DATA_DIR/update-bridge/state.json` после прерванного обновления.

**Решение:**

1. Обновитесь до **v1.1.50+** (автозакрытие stale-задач).
2. **Настройки → Целостность и восстановление → Применить исправления** — действие `clear_stuck_job`, или API:
...
3. При необходимости: `docker compose restart update-bridge`

**v1.1.51+:** перед новым обновлением `update-bridge` снимает зависшую активную задачу.

## Медленная или зависающая страница поста (v1.1.48+)

Большой объём телеметрии (сотни тысяч записей на пост) замедляет старые версии Dashboard, загружавшие всю телеметрию CRM в браузер.

**Решение:** **v1.1.48+** (индексы MongoDB), **v1.1.51+** (история поста: серверный фильтр, 100 строк, «Загрузить ещё» над таблицей, `count=false`). При необходимости — **Архив** для очистки старой телеметрии.
