...
| **Переключатель** | Шапка (флаги 🇺🇸/🇷🇺; на мобильных — один флаг) и **Настройки → Language** |
...
### Мобильная шапка (v1.1.13)

- переключатель языка — одна иконка с флагом текущей локали;
- Live/Static — одна иконка (радио / пауза); на `sm+` — сегментированный переключатель с подписями.
