...
| Пользователь | из `.env` → `MQTT_USER` (по умолчанию `wash`) |

Внутри Docker: `mosquitto:1883`.  
С локальной сети: `mqtt://wash:PASSWORD@192.168.1.10:1883` (подставьте IP сервера с CRM).
...
const client = mqtt.connect('mqtt://wash:PASSWORD@192.168.1.10:1883');
...
mosquitto_pub -h localhost -p 1883 -u wash -P 'PASSWORD' \
...
mosquitto_pub -h 192.168.1.10 -p 1883 -u wash -P 'PASSWORD' \
  -t 'wash/telemetry/mode' -q 1 \
  -m '{"postSerial":"POST-001","messageType":"mode","payload":{"mode":"idle"},"timestamp":"2024-06-22T12:00:00Z"}'
```
...
Порт 1883 доступен всем устройствам в локальной сети, которые могут достучаться до сервера CRM. Используйте сложный `MQTT_PASSWORD` и при необходимости ограничьте доступ файрволом.
